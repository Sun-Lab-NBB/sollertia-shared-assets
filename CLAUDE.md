...
- **Data layer**: `SessionData` (in `data_hierarchy/session_data.py`) is the entry point for every session on disk.
  `SessionData.create()` mints a new session, and `SessionData.load()` rehydrates one. Both build runtime-only
...
- **Persistent host settings**: `configuration/configuration_utilities.py` manages three independent
  ...
  its canonical filename lives in the top-level `credentials.py` module, downstream of `registries.py`, because its
...
- Apache-2.0 licensed, with the license string in `pyproject.toml` and mirrored in `LICENSE`
...
- Ruff for formatting and linting, with the project-specific lint ignores declared in `pyproject.toml`
