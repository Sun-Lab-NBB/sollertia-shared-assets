...
def test_dataset_data_create_writes_marker_after_descriptions(
    ...
    monkeypatch.setattr(target=DatasetData, name="save", value=_record_save)
...
def test_dataset_data_create_rolls_back_when_the_descriptions_write_fails(
    ...
    monkeypatch.setattr(target=DatasetData, name="_write_column_descriptions", value=_fail_descriptions)
...
def test_dataset_data_create_rolls_back_when_a_session_directory_cannot_be_created(
    ...
    monkeypatch.setattr(
        target=dataset_data_module, name="ensure_directory_exists", value=_make_failing_directory_creator(2)
    )
...
def test_dataset_data_create_rolls_back_when_the_marker_write_fails(
    ...
    monkeypatch.setattr(target=DatasetData, name="save", value=_fail_save)
...
def test_dataset_data_create_rolls_back_on_an_interrupt(
    ...
    monkeypatch.setattr(target=DatasetData, name="save", value=_interrupt_save)
...
def test_dataset_data_create_skips_the_rollback_when_no_directory_was_created(
    ...
    monkeypatch.setattr(
        target=dataset_data_module, name="ensure_directory_exists", value=_make_failing_directory_creator(1)
    )
    monkeypatch.setattr(target=dataset_data_module, name="delete_directory", value=_record_removal)
...
def test_dataset_data_create_leaves_a_colliding_dataset_intact(tmp_path: Path) -> None:
    ...
    # Every screen runs outside the rollback, so a rejected request never removes the dataset with which it collided.
...
def test_dataset_data_create_reports_a_rollback_that_leaves_the_hierarchy_behind(
    ...
    monkeypatch.setattr(target=DatasetData, name="save", value=_fail_save)
    monkeypatch.setattr(target=dataset_data_module, name="delete_directory", value=_leave_directory)
...
def test_dataset_data_create_reports_a_rollback_that_cannot_remove_the_hierarchy(
    ...
    monkeypatch.setattr(target=DatasetData, name="save", value=_fail_save)
    monkeypatch.setattr(target=dataset_data_module, name="delete_directory", value=_fail_delete)
...
def test_dataset_data_add_sessions_appends_and_materializes(tmp_path: Path) -> None:
    ...
    dataset_data = _make_hierarchy_dataset(
        tmp_path=tmp_path, sessions=(DatasetSession(session="2024-01-15-12-30-45-000001", animal="animal_a"),)
    )
...
def test_dataset_data_create_writes_descriptions_feather(tmp_path: Path) -> None:
    ...
    dataset_data = _make_dataset(tmp_path=tmp_path, descriptions=_COLUMN_DESCRIPTIONS)
...
def test_dataset_data_column_descriptions_roundtrip(tmp_path: Path) -> None:
    ...
    _make_dataset(tmp_path=tmp_path, descriptions=_COLUMN_DESCRIPTIONS)

    loaded = DatasetData.load(dataset_path=tmp_path / "test_dataset")
    assert loaded.column_descriptions() == _COLUMN_DESCRIPTIONS
...
def test_dataset_data_get_column_description_hit(tmp_path: Path) -> None:
    ...
    assert dataset_data.get_column_description("lick") == _COLUMN_DESCRIPTIONS["lick"]
...
def test_dataset_data_verify_data_descriptions_passes_when_all_columns_described(tmp_path: Path) -> None:
    ...
    dataset_data = _make_dataset(tmp_path=tmp_path, descriptions=_COLUMN_DESCRIPTIONS)
    _write_session_data(dataset_data=dataset_data, columns=("time_us", "lick"))
...
def test_dataset_data_verify_data_descriptions_allows_unused_descriptions(tmp_path: Path) -> None:
    """Verifies the check is one-directional: a described column that no session emits is permitted."""
    dataset_data = _make_dataset(tmp_path=tmp_path, descriptions=_COLUMN_DESCRIPTIONS)
    # Emits only a subset of the described columns. The unused 'lick' description must not trigger a violation.
    _write_session_data(dataset_data=dataset_data, columns=("time_us",))
...
def test_dataset_data_verify_data_descriptions_raises_on_undescribed_column(tmp_path: Path) -> None:
    ...
    dataset_data = _make_dataset(tmp_path=tmp_path, descriptions=_COLUMN_DESCRIPTIONS)
    _write_session_data(dataset_data=dataset_data, columns=("time_us", "lick", "mystery_column"))
...
# The stand-ins below that take 'self' are installed onto DatasetData methods by monkeypatch, which is why they carry
# an unannotated receiver rather than the module-level signature their placement here suggests. The rest replace
# module-level functions the dataset_data module imported.
